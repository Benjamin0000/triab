...
<div class="container">
    ...
    <div class="row">
        <div class="col-md-8 offset-md-2">
            <a href="{{route('eshop')}}" class="btn btn-sm btn-primary"><i class="fas fa-arrow-left"></i> Go back</a>
            <h5 class="text-center">Update shop</h5>
            <div class="text-center">
                <img width="100" src="{{Storage::url($shop->logo)}}" alt="">
            </div>
            <form id="create_shop_form">
                ...
                <div class="form-group">
                    ...
                    <input type="text" name="name" value="{{$shop->name}}" required placeholder="Shop name" class="form-control">
                </div>
                @method('put')
                <div class="form-group">
                    <div class="row">
                            ...
                            <div class="form-group">
                                ...
                                <select required class="form-control" name="category">
                                    ...
                                    @foreach($categories as $category)
                                        <option @selected($category->id == $shop->category_id) value="{{$category->id}}">{{$category->name}}</option>
                                    @endforeach
                                </select>
                            </div>
                        </div>
                        <div class="col-md-6">
                            <div class="form-group">
                                ...
                                <div><input type="file" name="logo" accept="image/jpeg, image/png, image/jpg, image/gif, image/webp"></div>
                            </div>
                        </div>
                    </div>
                </div>
                ...
                <div class="form-group">
                    ...
                    <textarea name="description" required placeholder="Enter a short description about your shop"  class="form-control">{{$shop->description}}</textarea>
                </div>
                @csrf 
                <h6>Shop location</h6>
                <div class="form-group">
                            ...
                            <div class="form-group">
                                ...
                                <select required id="state_select" name="state" class="form-control">
                                    ...
                                    @foreach(config('states') as $state)
                                        <option @selected($state['name'] == $shop->state) value="{{$state['name']}}">{{$state['name']}}</option>
                                    @endforeach 
                                </select>
                            </div>
                        </div>
                            ...
                            <div class="form-group">
                                ...
                                <select required id="city_select" name="city" class="form-control">
                                    @foreach($cities as $city)
                                        <option @selected($city['name'] == $shop->city) value="{{$city['name']}}">{{$city['name']}}</option>
                                    @endforeach 
                                </select>
                            </div>
                        </div>
                ...
                <div class="form-group">
                    ...
                    <input value="{{$shop->address}}" type="text" required placeholder="Enter shop address" class="form-control" name="address">
                </div>
                <p class="msg"></p>
                <div class="form-group">
...
<script>
    window.onload = ()=>{
        ...
        $(document).on('submit', '#create_shop_form',  (e)=>{
            ...
            $.ajax({
                type: 'POST',
                url: '{{route('eshop.update', $shop->id)}}', // Replace with your server endpoint
                data: formData,
